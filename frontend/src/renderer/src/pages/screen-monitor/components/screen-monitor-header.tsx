...
const ScreenMonitorHeader: React.FC<ScreenMonitorHeaderProps> = ({
  ...
  const hasSources = screenAllSources.length > 0 || appAllSources.length > 0
  const startDisabled = isMonitoring || !isToday || !hasPermission || !hasSources || !!sourceError
  const startPopoverContent =
    sourceError ||
    (!hasPermission
    ...
    <div className="flex justify-between items-start mb-3 flex-col md:flex-row">
      ...
      <div className="flex items-center ml-0 md:ml-6 mt-4 md:mt-0 justify-end">
        <Space>
          ...
          {!isMonitoring ? (
            <Popover content={startPopoverContent} disabled={!startDisabled || !startPopoverContent}>
              <Button
                type="primary"
                icon={<IconPlayArrow />}
                size="large"
